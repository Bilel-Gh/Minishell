...
void creat_cmd_list(t_commande *cmd, int nb_node)
{
	...
	while (i < nb_node)
	{
		cmd = malloc(sizeof(t_commande));//security!!!
		init_struc_cmd(cmd);
		cmd = cmd->next;
		i++;
	}
...
char **give_cmd_join(t_token *token, int nb_cdm)
{
    ...
    while (token_search)
    {
        while (token_search)
        {
            ...
            if (token_search->info->type == PIPE)
                break;
...
t_commande *cmd_complete(t_token *token)
{
  ...
  //  char **cmd_join;
    ...
    list_commande = malloc(sizeof(list_commande));
    if (!list_commande)
        return (list_commande);
    init_struc_cmd(list_commande);
    head = list_commande;
    creat_cmd_list(list_commande, nb_node);
    list_commande = head;
    give_cmd_join(token, nb_node);
